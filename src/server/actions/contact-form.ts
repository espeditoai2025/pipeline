...
export async function submitContactForm(data: unknown): Promise<ContactFormResult> {
  ...
  // Invia notifica interna a support@pipely.it
  if (resend) {
    await resend.emails.send({
      from: FROM_DEFAULT,
      to: SUPPORT_EMAIL,
      replyTo: email,
      subject: `[Pipely Contatti] ${subjectLabel} — ${name}`,
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
          <h2 style="color:#1e293b">Nuovo messaggio dal form contatti</h2>
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <tr><td style="padding:8px 0;color:#64748b;font-size:14px;width:120px">Nome</td><td style="padding:8px 0;font-size:14px;font-weight:600">${name}</td></tr>
            <tr><td style="padding:8px 0;color:#64748b;font-size:14px">Email</td><td style="padding:8px 0;font-size:14px"><a href="mailto:${email}">${email}</a></td></tr>
            <tr><td style="padding:8px 0;color:#64748b;font-size:14px">Argomento</td><td style="padding:8px 0;font-size:14px">${subjectLabel}</td></tr>
          </table>
  ...
  if (resend) {
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
          <h2 style="color:#1e293b">Ciao ${name},</h2>
          <p style="color:#475569;line-height:1.6">
            Grazie per averci contattato! Abbiamo ricevuto il tuo messaggio riguardo a
            <strong>${subjectLabel.toLowerCase()}</strong> e ti risponderemo al più presto,
            di solito entro 1 giorno lavorativo.
          </p>
          <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin:20px 0;font-size:14px;line-height:1.6;color:#334155;white-space:pre-wrap">${message}</div>
